...
export function CalibrationPoints() {
  const [xDim, yDim, xPoints, yPoints, rawHeightMap] = useSelector(
    (state: RootState) => [
  ...
  const heightMap = React.useMemo(() => rawHeightMap.flat(), [rawHeightMap]);

  const [points, setPoints] = React.useState<JSX.Element[]>([]);

  function drawPoints() {
    const xDelta = xDim / (xPoints - 1);
    const yDelta = yDim / (yPoints - 1);
