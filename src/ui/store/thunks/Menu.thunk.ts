...
import { calibrationActions, CALIBRATION_STATE } from '../reducers/Calibration.reducer';
...
//   });
  ipcRenderer.on('menu/openHeightMap', async (event, hmData: ReturnType<typeof heightMapSelector>) => {
    if ((getState() as RootState).calibration.state !== CALIBRATION_STATE.IDLE) {
